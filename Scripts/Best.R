...
best <- function(state, outcome)
{
  ...
  {
    #print(min(newdataframe$HA30,na.rm = TRUE))
    #print(newdataframe$Hospital.Name[newdataframe$HA30 == min(newdataframe$HA30,na.rm = TRUE)])
    #print(newdataframe[60,])
    #summary(newdataframe$HA30)
  
    # we get NAS and some real hospital names, we use unique to get only the notn repeated instances of both, store it in an vector  
     vecresult <- unique(newdataframe$Hospital.Name[newdataframe$HA30 == min(newdataframe$HA30,na.rm = TRUE)])
     filterNAS <- !is.na(vecresult) 
     vecresult <- vecresult[filterNAS]
     sort(vecresult,decreasing = FALSE)

  }
   else if(outcome == "heart failure")
   {
     #print(min(newdataframe$HF30,na.rm = TRUE))
      vecresult <- unique(newdataframe$Hospital.Name[newdataframe$HF30 == min(newdataframe$HF30,na.rm = TRUE)])
      #print(vecresult)
      filterNAS <- !is.na(vecresult) 
      vecresult <- vecresult[filterNAS]
      sort(vecresult,decreasing = FALSE)
  ...
  {
    # we get NAS and some real hospital names, we use unique to get only the notn repeated instances of both, store it in an vector  
    vecresult <- unique(newdataframe$Hospital.Name[newdataframe$PN30 == min(newdataframe$PN30,na.rm = TRUE)])
    filterNAS <- !is.na(vecresult) 
    vecresult <- vecresult[filterNAS]
    sort(vecresult,decreasing = FALSE)
